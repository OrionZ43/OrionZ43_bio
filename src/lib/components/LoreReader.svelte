<script lang="ts">
    ...
    // Ссылка на HTML-элемент со скроллом
    let contentArea: HTMLElement;

    // Реактивно получаем контент
    $: lang = $currentLang;
    $: currentChapter = loreChapters.find(c => c.id === activeChapterId)?.[lang] || loreChapters[0].ru;

    // МАГИЯ: Когда меняется ID главы, скроллим контейнер вверх
...
<div class="fixed inset-0 z-[60] bg-void/90 backdrop-blur-md flex items-center justify-center p-4 md:p-8" transition:fade={{ duration: 300 }}>
    ...
    >
        ...
        <!-- RIGHT SIDE: CONTENT -->
        <!-- Добавили bind:this={contentArea} к этому блоку -->
        <div
        ...
        >
            ...
            <!-- Styled Content -->
            <div class="p-6 md:p-10 pt-2 space-y-4">
